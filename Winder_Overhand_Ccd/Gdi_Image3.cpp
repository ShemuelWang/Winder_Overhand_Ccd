#include "StdAfx.h"
#include "Gdi_Image3.h"

//消息映射
BEGIN_MESSAGE_MAP(CGdi_Image3, CWnd)
	//{{AFX_MSG_MAP
	ON_WM_PAINT()//绘图函数
	ON_WM_LBUTTONDOWN()
	ON_WM_LBUTTONUP()
...
void CGdi_Image3::OnLButtonDown(UINT nFlags, CPoint point)
{
...
void CGdi_Image3::OnMouseMove(UINT nFlags, CPoint point)
{
	if (m_bDown)
	{
	...
	if (  abs(point.x - t_Rect.Width() ) <= 5 || abs(point.y - t_Rect.Height() ) <= 5 || point.x < 5 || point.y < 5 )
	{
		m_bDown =FALSE;
...
void CGdi_Image3::OnLButtonUp(UINT nFlags, CPoint point)
{
	if (m_bDown)
	{
		m_NMHDR.right = point.x;
...
BOOL CGdi_Image3::SubclassDlgItem(UINT nID, CWnd* pParent)
{
	//调用子类,关联对象与控件
...
void CGdi_Image3::Show(IPL v_Image)
{
	if (v_Image == NULL)
	{
	...
	{
		//根据控件尺寸缩放图像
		CGdi_Image3::Zoom(v_Image,&m_Image,t_Rect.Width(),t_Rect.Height() );

		Show_Direct(m_Image);
	}
}

void CGdi_Image3::Show_Direct(IPL v_Image)
{
	//定义矩形框
	CRect t_Rect;
...
BOOL CGdi_Image3::Zoom(const IPL v_Image, IPL *v_pTrans,int v_intWidth,int v_intHeight)
{
	if (v_Image == NULL)
	{
