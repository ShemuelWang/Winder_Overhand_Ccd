...
void BCMenuData::SetWideString(const wchar_t *szWideString)
{
	...
	if (szWideString)
    {
		...
		if (m_szMenuText)
			wcscpy_s(m_szMenuText,wcslen(szWideString),szWideString);
    }
	else
...
void BCMenu::MeasureItem( LPMEASUREITEMSTRUCT lpMIS )
{
	...
	if(state & MF_SEPARATOR){
		...
		if (Win32s!=g_Shell)
			...
			size.cx += 3*(size.cx/(int)(wcslen(lpstrText)) );
		}
#endif    
...
BOOL BCMenu::LoadMenu(LPCTSTR lpszResourceName)
{
	...
	do{
		...
		wcscpy_s(szCaption,wcslen((wchar_t *)pTp),(wchar_t *)pTp);
